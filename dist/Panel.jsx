...
export function ContainerPanel(props) {
    ...
    return <StyledPanelRoot style={{
        ...
        <StyledTabRow className={isActivePanel ? "active" : undefined}>
            <StyledTabRowInner draggable tabHeight={props.tabHeight} tabCount={panelRect.panel.contentList.length} onMouseDown={ev => {
            props.onClickPanel();
            props.onDragHeader(ev, null);
        }}>

                {panelRect.panel.contentList.map((content, tabNumber) => <StyledTab key={content.contentId} tabNumber={tabNumber} isCurrentTab={panelRect.panel.currentTabIndex == tabNumber} onMouseDown={ev => {
                props.onClickTab(tabNumber);
                props.onDragHeader(ev, tabNumber);
            }}>
                        <span>{content.title || `Content ${content.contentId}`}</span>
                        <StyledCloseButton isCurrentTab={panelRect.panel.currentTabIndex == tabNumber} onClick={ev => {
                props.onClickTab(tabNumber);
